    ...
    public void execute(Player player) {
        ...
        if (clan == null && plugin.getPermissionsManager().has(player, "simpleclans.leader.create")) {
            if (plugin.getSettingsManager().isePurchaseCreation()) {
                chatBlock.addRow(ChatColor.AQUA + "  " + MessageFormat.format(lang("0.create.tag.name.1.purchase.a.new.clan",player), clanCommand, ChatColor.WHITE));
            } else {
                chatBlock.addRow(ChatColor.AQUA + "  " + MessageFormat.format(lang("0.create.tag.name.1.create.a.new.clan",player), clanCommand, ChatColor.WHITE));
            }
        }
        ...
        if (isVerified && isLeader && plugin.getPermissionsManager().has(player, "simpleclans.leader.rank.create")) {
        	chatBlock.addRow(ChatColor.AQUA + "  " + MessageFormat.format(lang("0.rank.create.rank.1.create.rank",player), clanCommand, ChatColor.WHITE));
        }
